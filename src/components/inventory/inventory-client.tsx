...
import { ArrowDown, ArrowUp, RefreshCw, Plus, Loader2, AlertTriangle, TrendingUp, Package } from "lucide-react"
...
export function InventoryClient({ entries, products, pricingProducts }: {
  ...
  const [tab, setTab] = useState<"products" | "movements" | "pricing" | "alerts">("products")
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex gap-1 p-1 rounded-lg bg-muted w-fit flex-wrap">
          {(["products", "movements", "pricing", "alerts"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={cn("px-3 py-1.5 rounded-md text-sm font-medium transition-colors",
                tab === t ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
              )}>
              {t === "products" ? <span className="flex items-center gap-1.5"><Package className="w-3.5 h-3.5" /> Produtos</span>
                : t === "movements" ? "Movimentações"
                : t === "pricing" ? <span className="flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" /> Precificação</span>
                : (
        ...
        {tab === "products" && (
          <div className="space-y-3">
            ...
            <Card><CardContent className="p-0">
              <div className="hidden md:grid grid-cols-[2fr_1fr_1fr_1fr] gap-3 px-4 py-2.5 border-b bg-muted/40 rounded-t-xl">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Produto</span>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider text-right">Estoque atual</span>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider text-right">Mínimo</span>
                      ...
                      .map((p) => (
                        <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                          ...
                          <div className="w-24 text-right">
                            <p className="text-sm font-bold">{p.currentStock} un</p>
                          </div>
                          <div className="w-24 text-right">
                            <p className="text-sm text-muted-foreground">{p.minStock} un</p>
                          </div>
                          <div className="w-24 text-right">
                            <Badge variant={p.currentStock === 0 ? "destructive" : p.currentStock <= p.minStock ? "warning" : "success"} className="text-xs">
                              {p.currentStock === 0 ? "Sem estoque" : p.currentStock <= p.minStock ? "Baixo" : "OK"}
                            </Badge>
                          </div>
                        </div>
                      ))
